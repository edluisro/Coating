...
const services = [
  ...
  ["Responsive", "Diseno adaptable para movil, tablet y desktop desde el inicio."],
  ...
  return (
    <section className="services section section--alt" id="servicios" aria-labelledby="services-title">
      <div className="container">
        ...
        <Reveal className="services__grid" stagger={70}>
          {services.map(([title, text], index) => (
            <Card className="serviceCard" key={title}>
              <span className="serviceCard__icon" aria-hidden="true">
                {index + 1}
              </span>
              <h3>{title}</h3>
              <p>{text}</p>
              <a className="serviceCard__link" href="#contacto">
                Ver mas
              </a>
            </Card>
          ))}
